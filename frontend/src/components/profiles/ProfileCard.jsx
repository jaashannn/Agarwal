...
function ProfileCard({ profile }) {
  // Use the first image from profile.images, or fallback to placeholder
  const imageUrl = profile.images?.length > 0 ? profile.images[0] : 'https://via.placeholder.com/300x400';
  ...
  return (
    <Link to={`/profiles/${profile.id}`}>
      ...
      >
        ...
        <div className="p-6">
          <h3 className="text-xl font-playfair font-bold text-primary mb-1">{profile.name}</h3>
